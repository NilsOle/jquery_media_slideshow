...
(function ($) {
	...
	$.widget("thomaskahn.smoothDivScroll", {
		...
		options: {
			...
			scrollToEasingFunction: "easeOutQuart", // String  // dw ADAPTION

            dwScrollableAreaWidth: 0  // dw ADAPTION
		},
		_create: function () {
			...
			*****************************************/
			...
			// $(window).load(function () { // dw ADAPTION
		...
		_setElementScrollPosition: function (method, element) {
			...
			switch (method) {
				...
				case "id":
					...
					return false;
                case "data-dw-id": // dw ADAPTION
                    // Check that an element id is passed and that the element exists in the DOM
                    if (element.length > 0) {
                        if (el.data("scrollableArea").has("[data-dw-id='"+element+"']" ) && $(el).find("[data-dw-id='"+element+"']").length > 0) {
                            tempScrollPosition = $(el).find("[data-dw-id='"+element+"']").position().left;
                            el.data("scrollXPos", tempScrollPosition+(-940/2));
                            return true;
                        }
                    }
                    return false;
		...
		**********************************************************/
        ...
        move: function (pixels) {
            ...
            if ((pixels < 0 && el.data("scrollWrapper").scrollLeft() > 0) || (pixels > 0 && el.data("scrollableAreaWidth") > (el.data("scrollWrapper").innerWidth() + el.data("scrollWrapper").scrollLeft())) || o.manualContinuousScrolling ) {
                ...
                // !!!
                // console.log('1');
                // console.log((pixels < 0 && el.data("scrollWrapper").scrollLeft() > 0));
                // console.log('2');
                // console.log((pixels > 0 && el.data("scrollableAreaWidth") > (el.data("scrollWrapper").innerWidth() + el.data("scrollWrapper").scrollLeft())));
                // console.log('3');
                // console.log(o.manualContinuousScrolling);
                ...
                if( sOffset < 0 ) { // Swap last element to be the first one if scroll out of the left edge of view
                    ...
                    // dw ADAPTION
                    // Prevents mousewheel function from automatically jumping back and fourth
                    /*
		...
		**********************************************************/
		recalculateScrollableArea: function () {
            ...
            tempScrollableAreaWidth = o.dwScrollableAreaWidth; // dw ADAPTION
